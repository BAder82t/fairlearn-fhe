# Copyright (c) fairlearn-fhe contributors.
...
if HAS_FHE:
    ...
    # The default CKKS context has 8192 plaintext slots; subsample the
    # held-out set so this example fits in a single ciphertext (a
    # larger ``poly_modulus_degree`` would work too, at higher cost).
    eval_n = min(len(y_pred), 8192)
    y_test = y_test.iloc[:eval_n]
    sf_test = sf_test.iloc[:eval_n]
    y_pred = y_pred[:eval_n]
    print(f"Trained on {len(X_train)} rows, evaluating on {eval_n}.")
